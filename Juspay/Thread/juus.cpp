...
public:
    ...
    bool lockNode(const std::string &nodeLabel, int userId) {
        if (nodeMap.find(nodeLabel) == nodeMap.end()) return false; // Check if node exists
...
int main() {
    // Read the root label from input
    std::string rootLabel;
    std::cin >> rootLabel;

    // Create tree with root label
    MultiTree lockTree(rootLabel);

    // Read inputs and process queries
    readInputs(lockTree);
